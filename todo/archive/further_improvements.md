# Archive: Execute the CHANGELOG "Further improvements" backlog

Records items completed from `todo/further_improvements.md`. Item 7 — the first
release boundary, I-007 — is **not** here: it is still live in that note, ready
but blocked on live acceptance. Everything below is finished.

## 2026-07-27 — items 1 to 6

Split out of the live note on 2026-07-27, once each of these carried a verified
`Status: done` block. Reproduced below is the note as it stood, so the items
still read in their own context: the goal, the decisions the note proposed
(D35-D39, all since recorded in `docs/plan-gui-selective-sync.md` §1), the
constraints every item worked under, items 1 to 6 with their own status records,
what the note refused to do, and how it was verified. Headings are one level
deeper than in the original; the text is unchanged. Item 7 and the constraints it
still depends on stayed behind in
