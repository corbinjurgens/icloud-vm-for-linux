...
| D48 | Missing-share recovery route | The controller classifies D45's bounded helper text before dispatching a failure event: a credential marker means **credential rejected**; `mount error(2)` means **VM running but shares unavailable** and exposes **Set up Windows automatically** next to Retry; every other failed start means **VM did not start**. The missing-share action enters `PROVISIONING` in `first-run` mode from `START_FAILED`; the same first-run action is offered from `RUNNING` only when the latest health snapshot says both mounts are absent and Docker is definitively running. It writes the D39 first-run intent record before waiting for guest readiness, then stages the usual D43 run-specific record before a trigger. `lifecycle.py` remains a pure reducer: it receives only the classified event and performs no excerpt parsing or I/O. | A guest created outside the app, or one whose shares disappeared, otherwise has no route from a failed mount to provisioning: retry cannot create missing shares and normal reprovisioning was gated behind healthy mounts. The narrow `error(2)` diagnosis avoids treating every transient mount failure as permission to reset guest setup, while the drifted route requires both absent mounts plus Docker evidence rather than treating any red health state as proof. |
| D49 | App-created configuration (amends D31, D39, D41) | Setup defaults to **Create configuration**, which atomically creates `$XDG_CONFIG_HOME/icloud-bridge/env` (default `~/.config/icloud-bridge/env`) in a 0700 directory as a 0600 file. It contains editable, conservatively machine-derived `DISK_SIZE`, `RAM_SIZE` and `CPU_CORES` settings, plus a `secrets`-generated letters-and-digits-only `SHARE_PASS` of at least 24 characters. Creation uses exclusive create and never overwrites an existing file; a found conventional file is reused as the selection, including on resume, without persisting its path in D39. **Use an existing .env** remains the advanced manual path. This narrowly amends D41: the GUI may write the generated secret once at creation, but still never logs, displays, copies or re-reads it outside D41's existing delivery read points, never persists it anywhere else, and never rewrites an existing file's `SHARE_PASS`. | The Setup assistant must not send the operator to a terminal to invent a machine credential the app already transports safely. A conventional location makes a resume discoverable rather than remembered, while exclusive creation preserves operator-owned files and D39's no-path record. The generated alphabet survives the existing env-file and PowerShell quoting grammars. |
| D50 | The share's ACLs are the only permission authority on the data mount (extends D33) | `host/mnt-icloud.mount` adds **`noperm`**, which stops the host's cifs client from evaluating permissions locally; every request is issued and the guest's SMB server decides it. `host/mnt-icloud_bridge.mount` deliberately does **not** get it — that share's root carries no DOS read-only attribute, so nothing is being vetoed there. Nothing else changes: `uid`/`gid`/`file_mode`/`dir_mode` stay as they are (they still shape what `ls` reports), the D15 deny ACEs plus ABE still hide and protect excluded roots, D8 authentication is untouched, and no server-side setting moves. The visible consequences are stated rather than hidden: the sync root keeps *displaying* as `0555` while writes to it now succeed, and a DOS read-only file anywhere on the share is refused by the guest rather than pre-refused by the host | Creating a file or directory at the root of `/mnt/icloud` failed with `EACCES` that never reached the guest (live host, 2026-07-28): the iCloud sync root carries the DOS read-only attribute — normal for a Windows folder with a custom icon, and re-set by the shell if cleared — and the cifs client maps that attribute onto `0555`, then rejects the create itself, while subdirectories keep `dir_mode`'s `0775`. The same create and delete at the share root succeed over `smbclient` with the same `syncshare` credential, so the server was always willing; the operator simply could not make a top-level item, and `acceptance-tests.sh` section 5 writes its canary exactly there. `noperm` is the right tool rather than a blunt one because the check it removes is not enforcing anything real: this mount forces a single `uid=1000`/`gid=1000` and constant `file_mode`/`dir_mode`, so the mode the client tests is one the unit file invented, not a report of the NTFS ACL — and the server evaluates that ACL on every request regardless. The alternatives are worse: clearing the attribute in the guest fights the shell, which re-applies it, and would break the folder's own customization; relaxing the NTFS ACL would weaken the boundary D15 depends on; moving the canary off the root would hide the operator-visible defect instead of fixing it. The exposure this adds is bounded by what already exists — the credentials file is root-only, port 10445 is loopback-only, and one desktop user owns the mount — so the removed check protected nothing that the guest was not already protecting |
| D51 | The provisioning watcher's console is delegated back to conhost | The watcher points the `icloud` account's console delegation at **conhost.exe** (`DelegationConsole` and `DelegationTerminal` = `{B23D10C0-E52E-411E-9D5B-C09FDF709C7D}` under that user's `Console\%%Startup`), which honors the task action's `-WindowStyle Hidden`. The loop owns the setting, not just `-Install`: it is a per-user value and the poll loop is the only part of `watcher.ps1` that provably runs as `icloud`, so a guest built before this decision converges on its own. Writing it is idempotent and read back by re-applying; a change reports itself and the loop **exits once** so D42's one-minute keep-alive starts the new process with a conhost console, which is where the delegation actually takes effect. `-Install` additionally applies it when that profile's hive happens to be loaded, and never loads a hive of its own. A failure at any step is a warning: the watcher keeps working, visibly | Measured live on 2026-07-28 (`docs/acceptance-results.md`, F2): the task already passed `-WindowStyle Hidden`, but Windows 11 delegates every new console to Windows Terminal, which ignores the requested show state, so an elevated `WindowsTerminal` window rendered `==> Watching ...` forever and burned a constant **6.1-6.5% of one core** in a GPU-less VM - the single largest idle consumer, and a window whose close button killed the watcher. Alternatives lost on the same root cause or on D40: a `wscript`/VBS shim or `conhost.exe --headless` only restates the show state that the delegation layer is discarding (and both would have to rewrite the task action, which the re-provision path cannot deliver - it refreshes `watcher.ps1`, not the task definition); a non-interactive S4U principal is refused outright because D40 pins `LogonType Interactive` in the auto-logged-on `icloud` session for in-session elevated work, and an S4U token has no network identity for `\\host.lan\Provision` either. The blast radius is accepted and deliberately scoped to one hive: every console `icloud` starts gets conhost, which for a purpose-built provisioning account means our own watcher, orchestrator and agent windows - the agent's `-WindowStyle Hidden` becomes true for the first time as well. No OEM half in `01-debloat.ps1`: it would duplicate the GUID in a second file to save one minute on a path that already restarts, and could never reach an existing VM |
| D52 | Safe Workspaces: an optional local editing replica layered above D6's raw SMB mount | An opt-in, per-directory pairing of a chosen directory under `/mnt/icloud` with a real local-filesystem directory on the host, reconciled bidirectionally by **Unison 2.52 or newer** in finite one-shot cycles run through the GUI's existing worker pool. The full design — configuration and state schemas, path normalization, lifecycle gating, the two-consecutive-poll stability fingerprint that deliberately excludes `ctime`, the exact engine invocation and its forbidden options, the destructive-change guard, backup retention, conflict presentation, GUI strings, privacy boundaries, install paths, failure classification, and the live acceptance matrix — is `docs/plan-safe-local-workspaces.md`, which this row locks. The load-bearing constraints: it is **opt-in and per directory**, so existing users keep the raw mount with no copying; cycles run **only** while the GUI process is up and the lifecycle is in normal monitoring, are counted in the drain gate, and short-circuit on the `/var/lib/icloud-bridge/powered-off` marker before touching CIFS at all; conflicts are **retained on both replicas**, never resolved automatically (no `prefer`, no `force`, no `copyonconflict`, no `repeat`, no `ignorelocks`); central backups keep ten versions before every overwrite or deletion; a mass deletion halts behind a guard instead of propagating; and forgetting a workspace or removing the package deletes no replica, archive, log, or backup. This narrowly **amends §9**: the project-managed workspace replicator may be paused from the GUI, and only that. Apple's own iCloud sync remains unpausable and out of scope, exactly as before | The observed failure is an editor autosaving into a network filesystem whose metadata is rewritten out of band by the guest's Cloud Files filter: one note kept an unchanged content hash, size, inode and mtime while its `ctime` moved twice after a save (supporting evidence, not a captured cause-and-effect sequence), and Obsidian's own guidance warns that iCloud Drive on Windows can duplicate or corrupt files. Moving the editor onto local disk and putting a conflict-aware boundary in front of the mount removes that exposure without needing the root cause settled. **This does not reverse D6.** SMB remains the live guest-to-host transport and the source and destination of every remote-side cycle: Unison's remote replica *is* `/mnt/icloud`, the mount options of D33/D50 are untouched, Files On-Demand stays on and nothing is pinned (D14, D25), and a host without a Safe Workspace behaves exactly as it does today. A robocopy-style mirror is still refused for the reason D6 gave — this replica is bidirectional, archive-based and conflict-preserving, not a one-way poll. **This does not reverse §0's FUSE rejection either.** That rejection was of an always-on custom filesystem daemon in the data path; a Safe Workspace is a separate replica reconciled by short-lived, bounded, restartable processes that hold no mount, own no data path, and die with the GUI. Unison rather than a new engine because archive-based update detection, divergent-edit refusal and restartability are exactly what a mature synchronizer already has and what loses data when reimplemented; `rclone mount --vfs-cache-mode full` was rejected on its own documented warning that an out-of-band remote length change during attribute caching can expose truncated or garbage content — the same failure class being escaped |
| D53 | GUI palette-aware theme | One Qt-free `theme.py` module owns every colour the GUI paints. The Qt layer reads the desktop colour scheme, falls back to the palette when it is unknown, and reapplies persistent styles when the application palette changes; severity keys remain the existing `health` tokens | Hardcoded light-theme colours made severity dots and links illegible in dark sessions, inverted the excluded-row dimming, and encouraged disabling copyable labels merely to make them secondary. One pure palette model keeps the colours testable without Qt and makes the desktop switch a normal repaint rather than a restart requirement |
| D54 | Keyboard-selective sync and non-colour severity | Every mutating selective-sync action is reachable from the keyboard: Space, Enter and Return toggle the selected file or folder through the same check-state path as a mouse click, while Load more keeps its existing keyboard activation. Status health rows and Setup readiness rows pair the existing severity colour with a fixed green-circle, yellow-triangle or red-square glyph and matching accessible text | An included checkbox in a non-current tree column was unreachable from the keyboard, and dots that differed only by colour made a warning indistinguishable from a healthy result in a monochrome screenshot or for a colour-blind reader. Reusing the mouse path preserves the existing confirmation and D19 subsumption rules |
| D55 | Best-effort window presentation state | The GUI remembers its window geometry and selected tab in `$XDG_STATE_HOME/icloud-bridge-gui/window.json`. This is presentation state, never operator data: failures to read or write it are invisible to lifecycle startup, and a stored window wholly off the available screens is discarded. | A tray application opened repeatedly through the day should return to the operator's working layout, while a corrupt or stale local preference must never make the bridge inaccessible. |
...
| File | Writer | Purpose |
|---|---|---|
| `trigger.json` | host | Fixed, atomic-last request for one reconciliation run. |
| `status.json` | guest orchestrator | Per-run progress and result. |
| `watcher.json` | watcher | Best-effort presence beacon: task name, bundled agent build and registration/start time. `guestprov.read_watcher_beacon()` defensively validates it before staging; it is only evidence that a watcher registered or started at that time, never proof that it remains healthy or can accept a run. |
...
│   ├── theme.py           # Qt-free: palette-aware colours and pure scheme choice
│   ├── uistate.py         # Qt-free: best-effort XDG window presentation state
...
- **Health notifications (`notify.py`, Qt-free).** A colour change is silent, so
  ...
  is the surface.
  Only normal monitoring feeds the health reducer: the starting, shutting-down,
  provisioning and intentionally-powered-off states reset it and stop feeding
  it, so an expected red never announces itself. Provisioning separately
  notifies once per run when it is waiting for the operator to sign in, fails,
  or completes, because those are the moments the operator needs to return.
  After a successful `power_on`
...
- The selected file or folder's inclusion can be toggled with Space, Enter or
  Return. These keys use the same checkbox update path as a mouse click, so the
  D19 rules and the folder-wide include confirmation remain identical. Root and
  missing rows do not mutate; **Load more…** keeps its existing activation.
...
- **Startup precedes all CIFS I/O.** The controller pauses bridge I/O, runs the
  ...
  **Open VM screen**, and runs `power_on()` asynchronously. On success it enables
  ...
  VM screen** and **Retry start**, and leaves mount work paused. D48 classifies the
  D45 excerpt for the heading: VM did not start, VM running but shares unavailable,
  or credential rejected. Only `mount error(2)` also offers **Set up Windows
  automatically** as a first-run recovery (a minimized launch uses a tray
  notification, not an invisible modal).
...
- **First-run assistant (`firstrun.py`, Qt-free and mount-I/O-free) — D31.**
  ...
  2. **Creation and handoff.** **Create Windows VM** appears only with the
     ...
     diagnostic. Success enters **Provisioning Windows** and automatically
     starts the first guest run; it waits through Windows installation and then
     tells the operator when sign-in is needed, alongside the host command matching this install
...
- **In-session power control (D30).** The controller holds one lifecycle state —
  ...
  - `start_failed` → **Retry start**; D48's `mount error(2)` classification also
    exposes **Set up Windows automatically** and enters first-run provisioning;
...
- [ ] **E16** (D53) **Desktop theme legibility.** In both a light and a dark
  desktop session, read every banner, dot, link and excluded row and confirm it
  is legible. Then switch the desktop theme with the window open and confirm the
  window follows.
- [ ] **E17** (D54) **Keyboard-only selective sync and severity legibility.** On
  the real desktop, complete an exclude/apply cycle using only the keyboard, and
  confirm the Status and Setup rows remain distinguishable in a greyscale
  screenshot.
- [ ] **E18** (D55) **Window presentation restore.** On the real desktop, resize
  and move the window, switch tabs, quit and relaunch, and confirm both are
  restored; then corrupt `window.json` and confirm the app starts normally with
  defaults.
...
- Pause/resume **Apple's iCloud sync** from the GUI — leaving the VM and mounts
  up while iCloud stops syncing. D30's Power off/Start bridge is a different
  thing: a whole-bridge power operation that stops the VM and disconnects both
  shares. **Amended by D52**, and only this far: the *project-managed* Safe
  Workspace replicator may be paused per workspace from the GUI, which stops
  this project copying between a local replica and `/mnt/icloud` and changes
  nothing inside the guest. Apple's client keeps syncing whenever the guest is
  up; there is still no control over it here.
